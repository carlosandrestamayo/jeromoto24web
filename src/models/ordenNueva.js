...
export const orden = reactive({
    ...
    getSql:async()=>{
        return new Promise((resolve,reject)=>{
            ...
            for(let i =0; i < orden.listaServiciosTaller.length;i++){
                ...
                let o = orden.selectServiciosTallerOrden.find(sl=> sl.id == s.id);
    ...
    createOrden:async()=>{
        ...
        orden.getSql().then(str=>{
            gestion(Orden.url(), { params: { opcion: 2,sql:query,sql2:str } })
            .then(res => {
       ...
       /*await gestion(Orden.url(), { params: { opcion: 2,sql:query,sql2:str } })
            .then(res => {
