...
<template>  
	...
	<q-card-section class="bg-primary text-white">
        <div class="text-h6">Gestion Motocicleta con Placa {{moto.dato.name}}</div>
    ...
    <q-card-section  class="row" v-if="moto.listaTabla.length > 0" style="height:400px;overflow-y: scroll;">
        <q-list style="width:80%;margin:0 10%" padding bordered class="" >
            <q-item v-for="dato in moto.listaTabla" bordered>
                ...
                <q-item-section>
                    <q-item-label style="font-size: 16px" class="text-bold q-mb-xs">{{dato.name}}</q-item-label>
                    ...
                    <q-item-label class="text-bold">Realizar cada {{dato.tablaKm}} kms</q-item-label>
                    <div style="height:2px;background:black;" class="q-mt-md"></div>
